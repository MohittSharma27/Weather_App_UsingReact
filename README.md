# Weather_App_UsingReact

# 🌦️ Responsive Weather App
A sleek and responsive Weather App built using React.js, designed to deliver real-time weather updates with a clean and intuitive user interface. This app utilizes the OpenWeatherMap API (or your chosen weather API) to fetch accurate weather data and integrates location services to provide hyper-localized forecasts based on the user's current location.

# 📷 Screenshots
![image](https://github.com/MohittSharma27/Weather_App_UsingReact/blob/20164ad8b5609898351f5ab17f335215b154c008/ui.png)


# 🚀 Features
📍 Live Location Access: Automatically detects and uses the user's current geographical location to display relevant weather data.

🌐 Weather by City Search: Allows users to search for weather conditions in any city around the globe.

🌡️ Real-Time Weather Updates: Fetches current temperature, humidity, weather conditions, wind speed, and more.

📱 Responsive UI: Optimized for all screen sizes — from mobile devices to desktops — using modern CSS and responsive design techniques.

🔄 Refresh Functionality: Easily refresh weather data to get the latest updates.

# 🔧 Tech Stack
Frontend: React.js, JavaScript, HTML, CSS
APIs: OpenWeatherMap API
Geolocation: HTML Geolocation API

